...
import { useScrolled } from '@/lib/scroll-motion';
...
export const Header = () => {
  ...
  const headerSurface = cn(
    "sticky top-0 z-50 w-full border-b backdrop-blur transition-[background-color,border-color,box-shadow] duration-300",
    scrolled
      ? "border-border/80 bg-background/95 shadow-[0_8px_24px_-12px_rgba(0,0,0,0.6)] supports-[backdrop-filter]:bg-background/80"
      : "border-border/40 bg-background/60 supports-[backdrop-filter]:bg-background/50"
  );
  ...
  if (loading) {
    return <header className={cn("border-border/40 bg-background/60 backdrop-blur supports-[backdrop-filter]:bg-background/50 sticky top-0 z-50 w-full border-b")}>
  ...
  return <header className={headerSurface}>
      <div className="container flex h-14 items-center justify-between px-4">
        {/* Logo/Brand */}
        ...
        <div className="hidden md:flex items-center space-x-1">
          {[
            ...
            ...(!isGuest ? [{ label: 'Reports', path: '/my-reports', icon: 'User' }] : []),
            ...
            return (
              <Button
                ...
                onClick={() => navigate(isGuest && item.path === '/report' ? '/home' : item.path)}
              >
          ...
          <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
            ...
            <SheetContent side="right" className="w-80">
              <div className="flex flex-col space-y-4 mt-4">
                {/* User Info */}
                {isGuest ? (
                  <div className="flex flex-col items-center space-y-3 p-4 border rounded-lg text-center">
                    <div className="text-lg font-semibold">Welcome to FixTheMess</div>
                    <div className="text-sm text-muted-foreground">
                      Join to report civic issues and track them in your community.
                    </div>
