...
-- Add index for better performance on region filtering (skip existing ones)
CREATE INDEX IF NOT EXISTS idx_reports_region_id ON public.reports(region_id);
CREATE INDEX IF NOT EXISTS idx_reports_region_status ON public.reports(region_id, status);
